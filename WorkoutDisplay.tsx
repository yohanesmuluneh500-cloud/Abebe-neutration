    ...
    <div className="space-y-8 animate-in slide-in-from-bottom duration-500">
      <div className="glass p-6 rounded-xl">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-orange-600 hover:bg-orange-700 disabled:bg-zinc-800 text-white font-bold rounded-lg transition-all shadow-lg flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
      ...
      {workout && (
        <div className="space-y-6">
          <div className="flex justify-between items-end">
            ...
            <span className="text-zinc-500 font-bold text-xs pb-1">{workout.splitType}</span>
          </div>
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {workout.days.map((day, idx) => (
              <div key={idx} className="glass rounded-xl overflow-hidden border-t-2 border-zinc-700">
                <div className="bg-zinc-800/50 p-4 border-b border-zinc-700">
                  <h4 className="font-oswald font-bold text-xl text-orange-500">{day.dayName}</h4>
                </div>
                <div className="p-4 space-y-4">
                  {day.exercises.map((ex, eIdx) => (
                    <div key={eIdx} className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800/50 hover:border-zinc-700 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <h5 className="font-bold text-white text-lg">{ex.name}</h5>
                        <div className="flex gap-2">
                          <span className="px-2 py-1 bg-zinc-800 text-zinc-400 text-[10px] rounded font-bold">RPE {ex.rpe}</span>
