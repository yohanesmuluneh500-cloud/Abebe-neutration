    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-zinc-400">Coach Abebe is ready to push your limits. Consistency is the only path.</p>
        </div>
        
        {/* Watch Connectivity Status Indicator */}
        {watchData?.isConnected && (
          <div className="flex items-center gap-3 px-4 py-2 bg-orange-600/10 border border-orange-600/30 rounded-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className={`glass p-6 rounded-xl relative overflow-hidden group transition-all duration-500 border border-zinc-800/50 ${watchData?.isConnected ? 'border-orange-600/40 bg-orange-600/[0.03]' : ''}`}>
          <div className="flex justify-between items-start mb-6">
            <h3 className="text-zinc-500 font-bold text-xs tracking-widest uppercase">TRAINING TELEMETRY</h3>
            {watchData?.isConnected && <div className="text-[10px] font-bold text-orange-600 animate-pulse tracking-widest uppercase">LIVE SYNCING</div>}
          </div>

          <div className="space-y-6">
            {/* Heart Rate Section */}
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${watchData?.isConnected ? 'bg-orange-600/20 text-orange-500' : 'bg-zinc-800 text-zinc-600'}`}>
            ...
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${watchData?.isConnected ? 'bg-orange-600 text-white shadow-lg shadow-orange-900/20' : 'bg-zinc-800 text-zinc-600'}`}>
          ...
          <div className="mt-6 flex gap-1">
             <div className={`h-1 flex-1 rounded-full ${watchData?.isConnected ? 'bg-orange-600 animate-pulse' : 'bg-zinc-800'}`}></div>
             <div className={`h-1 flex-1 rounded-full ${watchData?.isConnected ? 'bg-orange-600/60 animate-pulse [animation-delay:0.2s]' : 'bg-zinc-800'}`}></div>
             <div className={`h-1 flex-1 rounded-full ${watchData?.isConnected ? 'bg-orange-600/30 animate-pulse [animation-delay:0.4s]' : 'bg-zinc-800'}`}></div>
          </div>
        </div>

        <div className="glass p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <svg className="w-20 h-20 text-orange-500" fill="currentColor" viewBox="0 0 24 24"><path d="M20.57 14.86L22 13.43L20.57 12L17 15.57L8.43 7L12 3.43L10.57 2L9.14 3.43L7.71 2L6.28 3.43L4.86 2L3.43 3.43L2 2L2 3.43L3.43 4.86L2 6.28L3.43 7.71L2 9.14L3.43 10.57L2 12L3.43 13.43L2 14.86L3.43 16.28L2 17.71L3.43 19.14L2 20.57L2 22L3.43 22L4.86 20.57L6.28 22L7.71 20.57L9.14 22L10.57 20.57L12 22L12 20.57L15.57 17L20.57 14.86Z" /></svg>
          </div>
        ...
        <div className="glass p-6 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
              <span className="text-orange-500 text-[10px] font-bold block mb-1 uppercase">PROTEIN</span>
      ...
      <section className="glass p-6 rounded-xl border-l-4 border-orange-600 bg-orange-600/5">
        <h3 className="text-xl font-oswald font-bold text-white mb-2 italic">ABEBE'S DAILY MANTRA</h3>
        <p className="text-zinc-300 italic leading-relaxed">"The pain of discipline is far less than the pain of regret. Leave your ego at the door, but bring your intensity. Every rep is a transaction with the future version of yourself."</p>
      </section>
    </div>
  );
};
